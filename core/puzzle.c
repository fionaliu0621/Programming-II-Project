...
static int parse_line(char *line, Puzzle *p)
{
    ...
    for (int i = 0; i < HINT_COUNT; i++)
    {
        tok = strtok(NULL, "|");
        if (tok)
            strncpy(p->hint[i], tok, sizeof(p->hint[i]) - 1);
        else
            strncpy(p->hint[i], "（無提示）", sizeof(p->hint[i]) - 1);
    }
...
int load_puzzles(const char *path, Puzzle out[], int max)
{
    ...
    if (!fp)
    {
        fprintf(stderr, "[puzzle] Cannot open %s\n", path);
        return -1;
    }
...
int calc_score(int elapsed_sec)
{
    if (elapsed_sec < 0)
        return 0;
    if (elapsed_sec <= HINT_INTERVAL * 1)
        return MAX_SCORE;
    if (elapsed_sec <= HINT_INTERVAL * 2)
        return MAX_SCORE - 1;
    if (elapsed_sec <= HINT_INTERVAL * 3)
        return MAX_SCORE - 2;
    if (elapsed_sec <= PUZZLE_TIME_SEC)
        return 1;
    return 0;
}
...
{
    if (!p || hint_index < 0 || hint_index >= HINT_COUNT)
        return NULL;
    if (!(p->hints_unlocked & (1u << hint_index)))
        return NULL;
